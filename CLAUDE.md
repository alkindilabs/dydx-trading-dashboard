...
In `buildYearReport`, the `_realizedFromFills` / `_realizedFillError` flags are kept as **audit annotations only** — they tell the operator whether a row's fill slice nets flat in isolation. The realized VALUE on the row is the continuous-FIFO attribution regardless of the flag, so totals reconcile to the equity curve even when individual slices look ragged. `warnings.positionsWithoutFifoCount` counts flagged rows for the panel's status strip.

**Reconciliation caveat — `invalid-fill-in-slice`.** `computeRealizedByFill` silently skips fills whose `price`/`size`/`side` won't parse (same behavior as `computeRealizedFromFills`). When a window contains such a fill, the FIFO walk's inventory state diverges from the chain's true state, and realized totals can be understated relative to the equity curve. The Tax panel surfaces this separately via `warnings.positionsWithInvalidFillCount` and the warning-strip text changes accordingly: it stops claiming totals are exact for the invalid-fill case and tells the operator the year total may be inaccurate. Reconciliation is exact only when every fill in the year's windows is parseable; in production dYdX data this is the norm, but the dashboard does not pretend otherwise on the edge.
