...
### Realized P&L source — continuous FIFO + per-fill attribution
Per-row realized P&L is derived by running `RiskMetrics.computeRealizedByFill` **once over the entire fills array** (continuous FIFO, no per-position window restart), then attributing each fill's realized contribution to the closed position whose `[openMs, closeMs]` window contains it. **Boundary rule:** when a fill timestamp matches `closeMs` of A and `openMs` of B, the fill is claimed by the position with the smallest `openMs` (the older-open of the two). This eliminates two bugs in the prior per-window-FIFO approach: (1) per-window FIFO restarted inventory at zero for each position window, which over- or under-counted realized whenever inventory state actually crossed position boundaries (heavily-scaled accounts), and (2) inclusive `[open, close]` windowing double-counted boundary fills (their fees AND their realized contributions) into both adjacent positions. The bug was material: on one live account (`dydx1agxtmjdgw7nrgs7m086m3qse7hwyf54gfeampv`, 2025), the per-window code reported +$28k profit while continuous FIFO reconciled to the equity-based −$77k loss.

The `/perpetualPositions.realizedPnl` field is NOT used as the source — it is known to undercount heavily-scaled accounts AND is mutated upstream by `RiskMetrics.normalizeRealizedPnl` before the Tax panel sees it (so the post-pipeline value is not a clean indexer reference). The rest of the dashboard's P&L pipeline already treats FIFO-from-fills as authoritative; see comment at `risk-metrics.js:548` and `index.html:1240`.

Helpers:
- `RiskMetrics.computeRealizedByFill(fills)` returns `{ total, byMarket, byFill }` where `byFill` is a `Map<fillObject, realized>` keyed by fill reference (so synthetic test fills without an `id` still attribute). Same FIFO walk as `computeRealizedFromFills` but records each fill's individual realized contribution.
- `TaxReport.realizedFromSlicedFills(position, fills)` — **legacy single-position helper** retained for external/unit-test callers that lack global context. Still emits `'no-fills-in-window'`, `'partial-fill-slice'`, or `'invalid-fill-in-slice'` errors with `realized: 0` per the old gate semantics. Production tax rendering goes through `buildYearReport`, not this helper.

In `buildYearReport`, the `_realizedFromFills` / `_realizedFillError` flags are audit annotations — they tell the operator whether a row's fill slice nets flat in isolation. The realized VALUE on the row is the continuous-FIFO attribution, so totals reconcile to the equity curve **provided every fill in the year's windows is FIFO-usable** (parseable price/size/side). `warnings.positionsWithoutFifoCount` counts flagged rows for the panel's status strip.

**Reconciliation caveat — invalid fills.** `computeRealizedByFill` silently skips fills whose `price`/`size`/`side` won't parse (same behavior as `computeRealizedFromFills`). When a window contains such a fill, the FIFO walk's inventory state diverges from the chain's true state, and realized totals can be understated. The Tax panel tracks this independently via `_hasInvalidFill` per row + `warnings.positionsWithInvalidFillCount`, branched separately from the net-flat gate: a row that is BOTH partial AND invalid still trips the invalid-fill counter. The warning strip swaps the "totals exact" wording for an "may be understated" warning in the invalid-fill case. Reconciliation is exact only when every fill in the year's windows is parseable; in production dYdX data this is the norm, but the dashboard does not pretend otherwise on the edge.

`buildYearReport` batch path: fills pre-grouped by market and sorted by `createdAtMs` once. Per-position window slicing uses binary search for `[lo, hi)` bounds (`O(log marketFills + windowSize)` per row). The continuous-FIFO walk and the fill-ownership map are built once per call (`O(n log n)` each, where n is total fills). Overlap detection is a sort+sweep per market (`O(n log n)` sort, `O(n + total_overlaps)` sweep) that records every chain member (A↔B↔C with A and C not overlapping still marks all three).

### Fee attribution
Per-fill fees on `/v4/fills` are attributed via the **same fill-ownership map** as realized P&L: each fill belongs to exactly one closed position (smallest-openMs claims it). **No side filter** — `/v4/fills` exposes side as `BUY`/`SELL` while positions are `LONG`/`SHORT`, AND both sides legitimately belong to a position's lifecycle (BUY opens a LONG / closes a SHORT; SELL closes a LONG / opens a SHORT). A direct side equality check would never match a single fill.

When ANOTHER closed position in the **same market** overlaps the window (regardless of side), the row carries `_feeAttributionWarning=true`. Under the new attribution scheme, fees are still attributed uniquely (no double-count of boundary fills), but the operator gets an audit hint that the overlap exists so they can sanity-check the row. The flag's name is historical; **realized P&L is just as ambiguous as fees** for overlap rows because the inventory crossover muddies per-position attribution. Downstream UI / CSV / JSON must treat the flag as covering BOTH fee and realized attribution.

Helper: `TaxReport.aggregateFeesForPosition(position, fills, closedPositions)` returns `{ totalFee, fillCount, warning }` — legacy single-position helper that still uses inclusive-window summation (no global attribution context). Production path goes through `buildYearReport`.

### Net realized P&L (per row)
`netUSD = realizedPnlUSD + netFundingUSD − feesUSD`. Fees always subtract because the continuous-FIFO source is provably gross of fees (`computeRealizedByFill` looks only at fill prices and sizes).

Helper: `TaxReport.netRealizedPnl(realizedPnlUSD, netFundingUSD, feesUSD)`.

### Currency conversion
USD → EUR via ECB daily reference rate on `closedDateUTC`. Source: `https://api.frankfurter.dev/v1/{date}?from=USD&to=EUR` (ECB-sourced, CORS-friendly, no auth). Weekend/holiday close dates inherit the nearest preceding business-day rate; the rate is stored in localStorage under the REQUESTED date so close-date lookups always hit. Cache key `fxRates:v1:USD-EUR`, indefinite TTL (historical reference rates do not change), independent of `dydxCache:v1` so `Forget` does not clear multi-year FX work.
...
Helper: `FxRates.getRates(dates: string[]) -> Promise<{rates, missing}>`. The `dates` array is built from the report's unique `closedDateUTC` values. The Tax panel only invokes `FxRates.getRates` when the Tax tab is actually active (`isTaxTabActive()` check in `render()`); users who never open the tab never hit api.frankfurter.dev/v1.
...
### Tests
`test/tax-report.test.js` pins year boundary in UTC, fee attribution (window + market, no side filter), per-fill realized attribution (boundary fill claimed by older-open position, not double-counted; continuous-FIFO realized surfaces across position boundaries even when neither slice is flat in isolation; heavy-scaling LONG reconciles to continuous-FIFO total), legacy per-window FIFO via `realizedFromSlicedFills`, no-fills-in-window flagged as `_realizedFromFills: false`, idempotent EUR conversion, summarize bucketing (including the `eurRowCount === 0 → undefined totals` rule), RFC 4180 CSV escaping. `test/fx-rates.test.js` exercises the cache + network paths against a stubbed `fetch`: cache hit, timeseries miss + persist, weekend single-date fallback, network-failure → `missing[]`, timeseries outage does NOT cascade into per-date fallback, concurrent `getRates` calls do not clobber each other's cache writes. The Playwright smoke (`test/smoke.spec.mjs`) routes `api.frankfurter.dev/v1` to a real-looking ECB-rate response for the fixture's close date so the Tax tab exercises the FX success path end-to-end, asserts the route was hit at least once, and waits for `#taxStatus` to leave the "Fetching ECB rates" state (deterministic) instead of a fixed timeout.
